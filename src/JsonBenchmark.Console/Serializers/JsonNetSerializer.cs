    ...
    public class JsonNetSerializer
    {
        ...
        public Model GetModelByIdAllInMemory(int id)
        {
            ...
            using (StreamReader r = new StreamReader(_jsonPath))
            {
                ...
                items = JsonSerializer.Deserialize<IEnumerable<Model>>(json);
            }
        ...
        public Model GetModelByIdIterateOnFile(int id)
        {
            ...
            using (var reader = new Utf8JsonStreamReader(fs, 32 * 1024))
            {
                while (reader.Read())
                {
                    if (reader.TokenType == JsonTokenType.StartObject)
                    {
                        var model = reader.Deserialize<Model>();
...
// From https://stackoverflow.com/a/55429664/4402936
